...
//   - Expects JSON response: {"res":"OK","data":"KEY=VAL,..."}
//   - Παίρνουμε S/N από το data field
// ---------------------
bool sjm_requestStatus(const String& ip, String& snOut) {
    ...
    // Parse JSON response: {"res":"OK","data":"KEY=VAL,..."}
    StatusInfo info;
    if (!parseStatusResponse(body, info)) {
        Serial.println("[STATUS] Failed to parse status response");
        return false;
    }
